...
func (l *Lab) StartInspection(ctx context.Context, i model.Inspection) (model.Inspection, error) {
	...
	gate, e := l.GetGate(ctx, i.GateID)
	if e != nil {
		return i, e
	}
	if gate.StationID != i.StationID {
		_ = l.store.Delete(kindStation, i.ID)
		return i, fmt.Errorf("%w: gate station mismatch", model.ErrInvalidCommand)
	}
