...
type Lab struct {
	...
	gateLocks map[string]*sync.Mutex
}

func NewLab(s *store.Store) *Lab {
	return &Lab{store: s, clock: clock.New(), closed: make(chan struct{}), gateLocks: map[string]*sync.Mutex{}}
}

func (l *Lab) gateLock(id string) *sync.Mutex {
	l.mu.Lock()
	defer l.mu.Unlock()
	if lock := l.gateLocks[id]; lock != nil {
		return lock
	}
	lock := &sync.Mutex{}
	l.gateLocks[id] = lock
