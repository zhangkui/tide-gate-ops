...
type Lab struct {
	...
	readingCache      []model.Reading
	readingCacheReady bool
}
...
func (l *Lab) ListReadings(ctx context.Context, stationID, kind string, from, to time.Time) ([]model.Reading, error) {
	if stationID == "" && kind == "" && from.IsZero() && to.IsZero() {
		l.mu.Lock()
		defer l.mu.Unlock()
		if !l.readingCacheReady {
			...
			l.readingCache = all
			l.readingCacheReady = true
		}
		return l.readingCache, nil
	}
	all, e := list[model.Reading](ctx, l.store, kindReading)
	if e != nil {
		return nil, e
