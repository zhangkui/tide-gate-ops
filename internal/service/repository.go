...
func (l *Lab) IngestReading(ctx context.Context, reading model.Reading) error {
	...
	sensor, err := l.GetSensor(context.Background(), reading.SensorID)
	if err != nil {
	...
	if err := save(context.Background(), l.store, kindReading, reading.ID, reading); err != nil {
		return fmt.Errorf("ingest reading: %w", err)
	}
	sensor.Online = true
	sensor.LastSeen = reading.ReceivedAt
	_ = save(context.Background(), l.store, kindSensor, sensor.ID, sensor)
	return l.store.AppendEvent(context.Background(), reading.StationID, "reading.ingested", reading)
}
