...
type Lab struct {
	...
	stationMiss map[string]bool
}

func NewLab(s *store.Store) *Lab {
	return &Lab{store: s, clock: clock.New(), closed: make(chan struct{}), stationMiss: map[string]bool{}}
}
